...
def ngalmemb(sig,r,mabscut):
    y=N.zeros(len(c.mass),'f')
    lbjmin=((Mbsol-mabscut)/2.5)
    #print "mabscut = ",mabscut
    for i in range(len(y)):
        membids=list(c.membids[i])
	dv=list(c.membdv[i])
	dr=list(c.membdr[i])
	lbj=list(c.memblbj[i])
...
def ngalobsmemb(sig,r,mabscut):
    y=N.zeros(len(c.mass),'f')# # of observed galaxies
    y2=N.zeros(len(c.mass),'f')#number of observed galaxies that are true members
    lbjmin=((Mbsol-mabscut)/2.5)
    #print "mabscut = ",mabscut
    for i in range(len(y)):
        membids=list(c.obsmembids[i])
	dv=list(c.obsmembdv[i])
	dr=list(c.obsmembdr[i])
	lbj=list(c.obsmemblbj[i])
	memb=list(c.membflag[i])
        for j in range(len(dv)):
            j=int(j)
            #if (g.memb[j] > 0):
	    if (abs(dv[j]) < sig):
...
def plothistdv():#plot histogram of memb/contam versus dv/sigma
    ...
    nr=1.
    bjmin=-18.
    llbjmin=((Mbsol-bjmin)/2.5)
    ppgplot.pgbeg("histdv2.ps/vcps",1,1)  #color port.
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgpage
    ppgplot.pgsch(1.2) #font size
    ppgplot.pgslw(4)   #line width
    ...
    #        gtype.append(g.type[int(l1[j])])
    #        obsmembdv.append(l3[j])
    membdv=N.compress((gmflag > 0.) & (glbj > llbjmin),gdv)
    contdv=N.compress((gmflag < 1.) & (glbj > llbjmin),gdv)
    latemembdv=N.compress((gltype > 0) & (gmflag > 0.) & (glbj > llbjmin),gdv)
    latecontdv=N.compress((gltype > 0) & (gmflag < 1.) & (glbj > llbjmin),gdv)
    #print membdv
    histdvsub2(membdv,contdv)
    ppgplot.pgtext(xlabel,.95,"All")
    linelabel(xlabel,ylabel,dx,ystep,dxl,dyl,"")
    ppgplot.pgsvp(x3,x4,y1,y2)  #sets viewport
    histdvsub2(latemembdv,latecontdv)
...
def linelabel2(xlabel,ylabel,dx,ystep,dxl,dyl,label):#draw key
    schdef=ppgplot.pgqch()
    ppgplot.pgsch(1.1)
    ppgplot.pgtext(xlabel,ylabel,label)
    ppgplot.pgsch(1.1)
    ...
    ppgplot.pgsls(1)
    ppgplot.pgline(xs,ys)
    ppgplot.pgtext((xlabel+dxl+dx),(ylabel-dyl),"All")
    ylabel=ylabel-ystep
    ys=ylabel*N.ones(len(xs),'f')
    ppgplot.pgsls(4)
    ppgplot.pgline(xs,ys)
    ppgplot.pgtext((xlabel+dxl+dx),(ylabel-dyl),"Late")
    ppgplot.pgsch(schdef)
    ppgplot.pgsls(1)
...
def histdvsub1(membid,membdv,contid,contdv):
    ...
    xmax=3.
    xmin=0.
    bins=N.arange(xmin,xmax,0.25)#make array of bins in step of 0.5mag
    nbin=len(bins)
    dv=abs(membdv)
    #print dv
    print "average dv = ",N.average(dv),min(dv),max(dv)
...
def plothistdmclcum():#plot histogram of memb/contam versus dv/sigma
    pylab.cla()
    pylab.clf()
    nv=3.
    nr=1.
    ...
    #gmcl=gmcl/100.
    membdv=N.compress((gmflag > 0.) & (gdr < nr) & (abs(gdv) < nv) & (glbj > llbjmin),gmcl)/100.
    contdv=N.compress((gmflag < 1.) & (gdr < nr) & (abs(gdv) < nv) & (glbj > llbjmin),gmcl)/100.
    latemembdv=N.compress((gltype > 0.1) & (gmflag > 0.) & (gdr < nr) & (abs(gdv) < nv) & (glbj > llbjmin),gmcl)/100.
    latecontdv=N.compress((gltype > 0.1) & (gmflag < 1.) & (gdr < nr) & (abs(gdv) < nv) & (glbj > llbjmin),gmcl)/100.

    dx=1.
    xmax=8.50+dx
    xmin=.50
    bins=N.arange(xmin,(xmax-dx),dx)#make array of bins in step of 0.5mag

    histdvsub2(membdv,contdv,latemembdv,latecontdv,bins)
    pylab.xlabel(r'$M_{Cl} \ (10^{12} \ M_\odot)$',fontsize=28,fontweight='heavy')

    pylab.axis([0.,9.,0.,.4])
    ax=pylab.gca()
    #pylab.setp(ax,yticklabels=['0.0','','0.2','','0.2','','0.3','','0.4','','0.6'])


    pylab.savefig('histdmclcumpy.eps')
def plothistdmagcum():#plot histogram of memb/contam versus dv/sigma
    pylab.cla()
    pylab.clf()
    nv=3.
    nr=1.
    ...
    #for i in range(20):
#	print bj[i]
    membdv=N.compress((gmflag > 0.) & (gdr < nr) & (abs(gdv) < nv),bj)
    contdv=N.compress((gmflag < 1.) & (gdr < nr) & (abs(gdv) < nv),bj)
    latemembdv=N.compress((gltype > 0.1) & (gmflag > 0.) & (gdr < nr) & (abs(gdv) < nv),bj)
    latecontdv=N.compress((gltype > 0.1) & (gmflag < 1.) & (gdr < nr) & (abs(gdv) < nv),bj)
    ...
    histdvsub3(membdv,contdv,latemembdv,latecontdv,bins)
    pylab.xlabel(r'$B_J}$',fontsize=28,fontweight='heavy')

    pylab.axis([-22.,-15,0.,.4])
    ax=pylab.gca()
    #pylab.setp(ax,yticklabels=['0.0','','0.2','','0.2','','0.3','','0.4','','0.6'])


    pylab.savefig('histdmagcumpy.eps')
def plothistdrcum():#plot histogram of memb/contam versus dv/sigma
    pylab.cla()
    pylab.clf()
    nv=3.
    nr=1.
    ...
    membdv=N.compress((gmflag > 0.) & (glbj > llbjmin) & (abs(gdv) < nv),gdr)
    contdv=N.compress((gmflag < 1.) & (glbj > llbjmin) & (abs(gdv) < nv),gdr)
    latemembdv=N.compress((gltype > 0.1) & (gmflag > 0.) & (glbj > llbjmin)  & (abs(gdv) < nv),gdr)
    latecontdv=N.compress((gltype > 0.1) & (gmflag < 1.) & (glbj > llbjmin) & (abs(gdv) < nv),gdr)
    ...
    histdvsub3(membdv,contdv,latemembdv,latecontdv,bins)
    pylab.xlabel(r'$\Delta r/R_{vir}}$',fontsize=28,fontweight='heavy')

    pylab.axis([0.,1.05,0.,.4])
    ax=pylab.gca()
    #pylab.setp(ax,yticklabels=['0.0','','0.2','','0.2','','0.3','','0.4','','0.6'])


    pylab.savefig('histdrcumpy.eps')


def plothistdvcum():#plot histogram of memb/contam versus dv/sigma
    pylab.cla()
    pylab.clf()
    nv=3.
    nr=1.
    bjmin=-18.
    llbjmin=((Mbsol-bjmin)/2.5)
    membdv=abs(N.compress((gmflag > 0.) & (glbj > llbjmin),gdv))
    contdv=abs(N.compress((gmflag < 1.) & (glbj > llbjmin),gdv))
    latemembdv=abs(N.compress((gltype > 0.1) & (gmflag > 0.) & (glbj > llbjmin),gdv))
    latecontdv=abs(N.compress((gltype > 0.1) & (gmflag < 1.) & (glbj > llbjmin),gdv))
    ...
    pylab.xlabel(r'$\Delta v/\sigma}$',fontsize=28,fontweight='heavy')
    ax=pylab.gca()
    pylab.axis([0.,6.05,0.,.4])

    #pylab.setp(ax,yticklabels=['0.0','','0.1','','0.2','','0.3','','0.4','','0.6'])
    pylab.savefig('histdvcumpy.eps')

def plothistdv05cum():#plot histogram of memb/contam versus dv/sigma
    pylab.cla()
    pylab.clf()
    nv=3.
    nr=0.75
    bjmin=-18.
    llbjmin=((Mbsol-bjmin)/2.5)
    membdv=abs(N.compress((gmflag > 0.) & (glbj > llbjmin) & (gdr < nr),gdv))
    contdv=abs(N.compress((gmflag < 1.) & (glbj > llbjmin) & (gdr < nr),gdv))
    latemembdv=abs(N.compress((gltype > 0.1) & (gmflag > 0.) & (glbj > llbjmin) & (gdr < nr),gdv))
    latecontdv=abs(N.compress((gltype > 0.1) & (gmflag < 1.) & (glbj > llbjmin) & (gdr < nr),gdv))
    ...
    ax=pylab.gca()
    pylab.axis([0.,6.05,0.,.4])
    pylab.text(.3,.17,'Late',fontsize=20)
    pylab.text(.3,.11,'All',fontsize=20)
    #pylab.setp(ax,yticklabels=['0.0','','0.1','','0.2','','0.3','','0.4','','0.6'])
    pylab.savefig('histdvr05cumpy.eps')


def histdvsub3(membdv,contdv,lmembdv,lcontdv,bins):
    dvall=list(membdv)+list(contdv)
    dvall=(N.array(dvall,'f'))
    ldvall=list(lmembdv)+list(lcontdv)
    ldvall=(N.array(ldvall,'f'))
    nbin=len(bins)
    (xbin,y1bin,y1binerr)=my.scipyhist2(bins,contdv)
    (x2bin,y2bin,y2binerr)=my.scipyhist2(bins,dvall)

    ybin=N.zeros(len(bins),'f')
    for i in range(len(bins)):
        try:
            ybin[i]=float(N.sum(y1bin[0:(i+1)]))/float(N.sum(y2bin[0:(i+1)]))
        except ZeroDivisionError:
            ybin[i]=0.
    my.drawhistpylab(bins,ybin,'k')
    (xbin,y1bin,y1binerr)=my.scipyhist2(bins,lcontdv)
    (x2bin,y2bin,y2binerr)=my.scipyhist2(bins,ldvall)
    ybin=N.zeros(len(bins),'f')
    for i in range(len(bins)):
        try:
            ybin[i]=float(N.sum(y1bin[0:(i+1)]))/float(N.sum(y2bin[0:(i+1)]))
            ...
            ybin[i]=0.
    my.drawhistpylab(bins,ybin,'k--')
    #pylab.legend( ('All', 'Late'),loc='upper left')
    pylab.ylabel(r'$\rm{Contamination }$',fontsize=28,fontweight='bold')

def histdvsub2(membdv,contdv,lmembdv,lcontdv,bins):
    dvall=list(membdv)+list(contdv)
    dvall=(N.array(dvall,'f'))
    ldvall=list(lmembdv)+list(lcontdv)
    ldvall=(N.array(ldvall,'f'))
    ...
    for i in range(len(bins)):
        try:
            ybin[i]=float(y1bin[i])/float(y2bin[i])
        except ZeroDivisionError:
            ybin[i]=0.
    my.drawhistpylab(bins,ybin,'k')
    (xbin,y1bin,y1binerr)=my.scipyhist2(bins,lcontdv)
...
class Cluster:
    ...
    def getmemberidsall(self,nv,nr,cg):#get galaxy members
        ...
        self.membdv=N.zeros(len(self.mass),'f')
        self.membdv=list(self.membdv)
        self.membdr=N.zeros(len(self.mass),'f')
        self.membdr=list(self.membdr)
        self.memblbj=N.zeros(len(self.mass),'f')
        self.memblbj=list(self.memblbj)
	    ...
	    magtemp=[]
            ...
            (membids,matchflag)=my.findmatch(self.id[i],cg.ih,deltaid)#g.ih is already sorted
            #if i < 5:
            #    print i,self.id[i],membids,self.membids[i],matchflag
	    ...
	    #print 'cdtheta = ',cdtheta
                ...
                if dr < nr:
                    ...
                    if dv <= nv*N.sqrt(3.)*cg.hsigmax[j]:
                        ...
                        if gdtheta < cdtheta:#make sure galaxy is in cone field-of-view
                            self.membincut[i]=1
			    drtemp.append(dr)
			    dvtemp.append(dv/(N.sqrt(3.)*cg.hsigmax[j]))
			    mtemp.append(j)
			    magtemp.append(cg.lbj[j])
		    ...
		    #print "looping over memberids"
				...
				#print "made vel cut"
                    ...
                    continue
	    self.membdr[i]=drtemp
	    self.membdv[i]=dvtemp
	    self.membids[i]=mtemp
	    self.memblbj[i]=magtemp
	    #print "cluster ",i," self.membincut[i] = ",self.membincut[i],len(self.membids[i]),dvtemp[0:10],drtemp[0:10]
    def getobsmemberidsall(self,nv,nr,cg):#get galaxies within nr*dr and nv*dv
        ...
        self.obsmembdv=list(self.obsmembdv)
        self.obsmembdr=N.zeros(len(self.mass),'f')
        self.obsmembdr=list(self.obsmembdr)
        self.obsmemblbj=N.zeros(len(self.mass),'f')
        self.obsmemblbj=list(self.obsmemblbj)
    ...
    def writegalaxyfile(self):
	print "writing galaxy file"
	out=file('allobsgalaxies.cat','w')
	out.write("#Mcl sigma gal-dv gal-dr gal-lbj gal-type(1=late) gal-memb(1=memb) \n") 
	for i in range(len(self.mass)):
	    membids=list(self.membids[i])
	    dv=list(self.obsmembdv[i])
	    dr=list(self.obsmembdr[i])
	    lbj=list(self.obsmemblbj[i])
	    ltype=list(self.typeflag[i])
	    mflag=list(self.membflag[i])
	    for j in range(len(dv)):
		s="%5.3e %5.3e %5.3e %5.3e %5.3e %5.3e %5.3e\n" % (self.mass[i],self.sigma[i],dv[j],dr[j],lbj[j],ltype[j],mflag[j])
		out.write(s)
	out.close()

	out=file('allmembgalaxies.cat','w')
	out.write("#Mcl sigma gal-dv gal-dr gal-lbj gal-type(1=late) gal-memb(1=memb) \n") 
	for i in range(len(self.mass)):
	    dv=list(self.membdv[i])
	    dr=list(self.membdr[i])
	    lbj=list(self.memblbj[i])
	    for j in range(len(dv)):
		s="%5.3e %5.3e %5.3e %5.3e %5.3e\n" % (self.mass[i],self.sigma[i],dv[j],dr[j],lbj[j])
		out.write(s)
	out.close()


    def getalldata(self,cg):#get member and observed ids within 6 sig and 3 Rv
	nv=6
	nr=1
        self.getmemberidsall(nv,nr,cg)
	#for i in range(len(self.mass)):
	#    print i,"membids = ",self.membids[i]
        self.getobsmemberidsall(nv,nr,cg)
	#for i in range(len(self.mass)):
	#    print i,"obsmembids = ",self.obsmembids[i]
	self.writegalaxyfile()

    def calculatecontam(self):
	(self.cont60518,self.compl6118)=self.measurecontam(6.,.5,-18.)
	self.printresults(6.,.5,-18.,self.cont60518)
	(self.cont30518,self.compl3118)=self.measurecontam(3,.5,-18.)
	self.printresults(3.,.5,-18.,self.cont30518)
	(self.cont60517,self.compl6118)=self.measurecontam(6,.5,-17.)
	self.printresults(6.,.5,-17.,self.cont60517)
	(self.cont30517,self.compl3118)=self.measurecontam(3,.5,-17.)
	self.printresults(3.,.5,-17.,self.cont30517)
	(self.cont60516,self.compl6118)=self.measurecontam(6,.5,-16.)
	self.printresults(6.,.5,-16.,self.cont60516)
	(self.cont30516,self.compl3118)=self.measurecontam(3,.5,-16.)
	self.printresults(3.,.5,-16.,self.cont30516)
	(self.cont6118,self.compl6118)=self.measurecontam(6,1.,-18.)
	self.printresults(6.,1.,-18.,self.cont6118)
	(self.cont3118,self.compl3118)=self.measurecontam(3,1.,-18.)
	self.printresults(3.,1.,-18.,self.cont3118)
    def printresults(self,nv,nr,mag,cont):
	ave=N.average(cont)
	std=pylab.std(cont)
	s="average contaminatio within "+str(nr)+"Rv, +"+str(nv)+" sig, MB<-"+str(mag)+" = %8.3f +/- %8.3f" % (ave,std)
	print s

    def measurecontam(self,nv,nr,magcut):
        print "within measurecontam(), nv,nr,len(c.mass) = ",nv,nr,len(self.mass)
	compl=N.zeros(len(self.mass),'f')
	cont=N.zeros(len(self.mass),'f')
	nmemb=ngalmemb(nv,nr,magcut)
	#for n in nmemb:
	#    print "ngal memb nmemb = ",n
	(nobs,nobsmemb)=ngalobsmemb(nv,nr,magcut)

	for i in range(len(self.mass)):
	    if nobs[i] > 0.:
		cont[i]=(nobs[i]- nobsmemb[i])/nobs[i]
	    if nmemb[i] > 0.:
		compl[i]=nobsmemb[i]/nmemb[i]
	return cont,compl
...
def readgalaxyfiles():
    print "mode = 1: reading galaxy catalogs"
    infile=file('allobsgalaxies.cat','r')
    mcl=[]
    sig=[]
    dv=[]
    dr=[]
    ...
    gdr=N.array(dr,'f')
    glbj=N.array(lbj,'f')
    gltype=N.array(ltype,'f')
    gmflag=N.array(mflag,'f')
    print "number of galaxies = ",len(gltype)," number of latetype = ",N.sum(gltype)
    return gmcl,gsig,gdv,gdr,glbj,gltype,gmflag
...
if (df == 1):
	...
	y1=c.cont6118
	y2=c.compl6118
	(xbin,ybin,ybinerr)=my.biniterr(c.mass,y1,nbin)
	pylab.plot(xbin,ybin,'ro')
	pylab.errorbar(xbin,ybin,ybinerr,fmt=None,ecolor='k')
	(xbin,ybin,ybinerr)=my.biniterr(c.mass,y2,nbin)
	pylab.plot(xbin,ybin,'go')
	pylab.errorbar(xbin,ybin,ybinerr,fmt=None,ecolor='k')
	print "Average contamination within 6 sigma = ",N.average(y1)

	y1=c.cont3118
	y2=c.compl3118
	(xbin,ybin,ybinerr)=my.biniterr(c.mass,y1,nbin)
	pylab.plot(xbin,ybin,'r^')
	pylab.errorbar(xbin,ybin,ybinerr,fmt=None,ecolor='k')
	(xbin,ybin,ybinerr)=my.biniterr(c.mass,y2,nbin)
	pylab.plot(xbin,ybin,'g^')
	pylab.errorbar(xbin,ybin,ybinerr,fmt=None,ecolor='k')
	print "Average contamination within 3 sigma = ",N.average(y1)
	ax=pylab.gca()
	ax.set_xscale("log")
	pylab.savefig('sdss.eps')
    (gmcl,gsig,gdv,gdr,glbj,gltype,gmflag) = readgalaxyfiles()
    #plotcontam()
    plothistdv05cum()
    maxsig=max(gsig)
    print "max value of sigma = ", maxsig
    print "corresponding +/- 6 sigma volume = ", 12*maxsig/70.
    print "300. * 70/12 = ", 300.*70./12.
    #plotmagcuts()
