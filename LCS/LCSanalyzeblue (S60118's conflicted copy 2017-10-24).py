...
def comparedata(rdata, cdata,rlabels=None,clabels=None,norm=False,scale=False,showTicks=True):
    ...
    fig = pl.figure()
    pl.subplots_adjust(bottom=.15,left=.15,wspace=.04,hspace=.04)
...
class spirals():
    def __init__(self,infile,usecoma=True,useherc=True,onlycoma=False,prefix='all'):
        ...
        if (usecoma == True) | (usecoma == False):
            ...
            flag=(self.sampleflag  & self.gim2dflag & (self.s.B_T_r == self.s.B_T_r) )
    ...
    def printsizeblue(self):
        ...
        pl.legend(loc='upper left',numpoints=1,fontsize=16)
        pl.axis([-.4,2.4,.6,1.6])
        pl.xticks(arange(3),names,fontsize=22)
        #pl.gca().set_yscale('log')
        pl.ylabel('$R_e(24)/R_e(r) $')
        pl.savefig(homedir+'research/LocalClusters/SamplePlots/sizeenvblue.eps')
        #pl.yticks(arange(0,2,.4))
        '''
        for i in range(len(allax)):
            pl.sca(allax[i])
    ...
    def plotsizesystematics(self,showTicks=False):
        ...
        datar=np.array([(self.s.Rhlr_2[flag]/self.DA[flag]),self.s.ng[flag],self.nsamag[:,4][flag]])#,log10(self.s.SFR_ZDIST[flag])])
    ...
    def plotsizedvdr(self,plotsingle=1,reonly=1,onlycoma=0,plotHI=0,plotbadfits=0,lowmass=0,himass=0,cluster=None,plothexbin=True,hexbinmax=40,scalepoint=0,clustername=None,blueflag=False,plotmembcut=True,colormin=.2,colormax=1,colorbydensity=False,plotoman=False):
        ...
        if USE_DISK_ONLY:
            clabel=['$R_e(24)/R_d$','$R_{iso}(24)/R_{iso}(r)$']
        else:
        ...
        if plotmembcut:
            ...
            xl=np.arange(0,2,.1)
            pl.plot(xl,-4./3.*xl+2,'k-',lw=3)
            #pl.plot(xl,-3./1.2*xl+3,'k-',lw=3)       
        ...
        if plotsingle:
            savefig(figname+'.png')
            savefig(figname+'.eps')
            pl.savefig(homedir+'Dropbox/Research/MyPapers/LCSpaper1/submit/resubmit/fig4.eps')
    ...
    def plotsizeBT(self,scalepoint=0,blueflag=False):
        ...
        for i in range(4):
            ...
            if i == 0:
                a.set_xticklabels(([]))
                pl.text(0.1,0.9,'$Core$',transform=a.transAxes,horizontalalignment='left',fontsize=20)
            ...
            if i == 2:
                text(0.1,0.9,'$Exterior$',transform=a.transAxes,horizontalalignment='left',fontsize=20)
    ...
    def plotsizeBTblue(self,scalepoint=0,blueflag=False,medianflag=False,diskonly=False):
        ...
        for i in range(4):
            ...
            #pl.plot(x[x_flags[i]],y[x_flags[i]],'kx',markersize=8,label='No Fit')
            if (i == 0) | (i == 2):
                pl.errorbar(x[point_flags[i]],y[point_flags[i]],yerr=yerror[point_flags[i]],fmt=None,ecolor='k')
                sp=pl.scatter(x[point_flags[i]],y[point_flags[i]],c=self.logstellarmass[point_flags[i]],vmin=mstarmin,vmax=mstarmax,cmap='jet',s=size,label='GALFIT')
                ...
                pl.text(.95,.9,r'$\rho = [%4.2f, %4.2f]$'%(np.percentile(rho,16),np.percentile(rho,84)),horizontalalignment='right',transform=ax.transAxes,fontsize=16)
                pl.text(.95,.8,'$p = [%5.4f, %5.4f]$'%(np.percentile(p,16),np.percentile(p,84)),horizontalalignment='right',transform=ax.transAxes,fontsize=16)
            ...
            if (i == 1) | (i == 3):
                ...
                pl.scatter(xbin,ybin,c=sbin,s=300,cmap='jet',vmin=mstarmin,vmax=mstarmax,marker='s')
            if (i == 1):
                ...
                #print yl
                plot(xl,yl,'k--')
                pl.text(0.1,0.9,'$Core$',transform=gca().transAxes,horizontalalignment='left',fontsize=20)
            if (i == 3):
                plot(xl,yl,'k--')
                #plot(xl,yl+.12,'b:')
                ...
                text(0.1,0.9,'$Exterior$',transform=gca().transAxes,horizontalalignment='left',fontsize=20)
            ...
            if i == 1:
                ...
                if medianflag:
                    ...
                    pl.title('$Weighted \ Mean $',fontsize=22)                   
            if i == 2:
                #text(0.1,0.9,'$Exterior$',transform=a.transAxes,horizontalalignment='left',fontsize=20)
                text(-0.2,1.,'$R_e(24)/Re(r)$',transform=a.transAxes,rotation=90,horizontalalignment='center',verticalalignment='center',fontsize=24)
    ...
    def plotsize3panel(self,logyscale=False):
        ...
        x=[self.logstellarmass,self.s.B_T_r,np.log10(self.s.SIGMA_5)]
        xlabels=['$\log(M_\star/M_\odot)$','$B/T$','$\Sigma_5 \ (gal/Mpc^2)$']
        colors=[self.s.B_T_r,self.logstellarmass,self.s.B_T_r]
        cblabel=['$B/T$','$\log(M_\star/M_\odot)$','$B/T$']
        cbticks=[np.arange(0,.7,.2),np.arange(8.5,10.8,.4),np.arange(0,.7,.2)]
        xticklabels=[np.arange(8.5,11.5,1),np.arange(0,1,.2),np.arange(-1.2,2.2,1)]
        xlims=[(8.3,11.3),(-.05,.9),(-1.2,2.2)]
        v1 = [0,8.5,0]
        v2 = [0.6,10.8,0.6]
        y=self.sizeratio
        yerror=self.sizeratioERR
        ...
        for i in range(len(x)):
            allax=[]
            for j in range(3):
                pl.subplot(nrow,ncol,3.*i+j+1)
                pl.errorbar(x[i][flags[j]],y[flags[j]],yerr=yerror[flags[j]],fmt=None,ecolor='.5',markerfacecolor='white',zorder=1)
                sp=pl.scatter(x[i][flags[j]],y[flags[j]],c=colors[i][flags[j]],vmin=v1[i],vmax=v2[i],cmap='jet',s=40,label='GALFIT')
                if j == 0:
                ...
                if i == 0:
                    if j == 0:
                        pl.title('$All $',fontsize=24)
                    elif j == 1:
                        pl.title('$Core$',fontsize=24)
                        
                    elif j == 2:
                        pl.title('$Exterior$',fontsize=24)
                if j == 1:
                    pl.xlabel(xlabels[i])
                if j == 0:
                    #pl.ylabel('$R_e(24)/Re(r)$')
                    pl.ylabel('$R_{24}/R_d$')
                if logyscale:
                    a.set_yscale('log')
                    ylim(.08,6)
        ...
        savefig(homedir+'Dropbox/Research/MyPapers/LCSpaper1/submit/resubmit/fig12.eps')
    def plotsizeNUVrblue(self,scalepoint=0,blueflag=False,medianflag=False,diskonly=False):
        ...
        for i in range(4):
            ...
            if (i == 0) | (i == 2):
                pl.errorbar(x[point_flags[i]],y[point_flags[i]],yerr=yerror[point_flags[i]],fmt=None,ecolor='k')
                sp=pl.scatter(x[point_flags[i]],y[point_flags[i]],c=color[point_flags[i]],vmin=colormin,vmax=colormax,cmap='jet',s=size,label='GALFIT')
                #print x[point_flags[i]],y[point_flags[i]],yerror[point_flags[i]]
                (rho,p)=spearman_with_errors(x[point_flags[i]],y[point_flags[i]],yerror[point_flags[i]])
            ...
            if (i == 1) | (i == 3):
                ...
                #xbin,sbin,sbinerr=my.binit(self.logstellarmass[point_flags[i]],self.sizeratio[point_flags[i]],5)
                errorbar(xbin,ybin,yerr=ybinerr,fmt=None,color='k',markersize=16)
                pl.scatter(xbin,ybin,c=sbin,s=300,cmap='jet',vmin=colormin,vmax=colormax,marker='s')
            if (i == 1):
    ...
    def plotsizestellarmassblue(self,scalepoint=0,blueflag=False,medianflag=False):
        ...
        for i in range(4):
            ...
            if (i == 0) | (i == 2):
                pl.errorbar(x[point_flags[i]],y[point_flags[i]],yerr=yerror[point_flags[i]],fmt=None,ecolor='k')
                sp=pl.scatter(x[point_flags[i]],y[point_flags[i]],c=pcolor[point_flags[i]],vmin=BTmin,vmax=BTmax,cmap='jet',s=size,label='GALFIT')
                (rho,p)=spearman_with_errors(x[point_flags[i]],y[point_flags[i]],yerror[point_flags[i]])
            ...
            if (i == 1) | (i == 3):
                ...
                #xbin,sbin,sbinerr=my.binit(self.logstellarmass[point_flags[i]],self.sizeratio[point_flags[i]],5)
                errorbar(xbin,ybin,yerr=ybinerr,fmt=None,color='k',markersize=16)
                pl.scatter(xbin,ybin,c=sbin,s=300,cmap='jet',vmin=BTmin,vmax=BTmax,marker='s')
    ...
    def plotssfrsigmair(self,scalepoint=0,usesize=0):
        ...
        for i in range(4):
            ...
            #pl.plot(x[x_flags[i]],y[x_flags[i]],'kx',markersize=8,label='No Fit')
            if usesize:
                sp=pl.scatter(x[point_flags[i]],y[point_flags[i]],c=self.sizeratio[point_flags[i]],vmin=.1,vmax=1,cmap='jet_r',s=size,label='GALFIT')
            else:
                sp=pl.scatter(x[point_flags[i]],y[point_flags[i]],c=self.logstellarmass[point_flags[i]],vmin=mstarmin,vmax=mstarmax,cmap='jet',s=size,label='GALFIT')
    ...
    def plotsizedvdr4(self,scalepoint=0):
        ...
        for i in range(4):
            ...
            sp=pl.scatter(x[point_flags[i]],y[point_flags[i]],s=size[point_flags[i]],vmin=.1,vmax=1,cmap='jet_r',c=self.sizeratio[point_flags[i]],label='GALFIT')
    ...
    def plotsalimcolormag(self):
        ...
        for i in range(4):
            ...
            pl.axhline(y=4.9,ls=':',color='k')
            pl.plot(x[x_flags[i]],y[x_flags[i]],'kx',markersize=8,label='No Fit')
            sp=pl.scatter(x[point_flags[i]],y[point_flags[i]],c=self.sizeratio[point_flags[i]],vmin=0.1,vmax=1,cmap='jet_r',s=100,label='GALFIT')
    ...
    def plotSFRStellarmassSize(self,clustername=None):
        ...
        for i in range(4):
            ...
            sp=pl.scatter(self.logstellarmass[point_flags[i]],y[point_flags[i]],c=self.sizeratio[point_flags[i]],vmin=0.1,vmax=1,cmap='jet_r',s=60,label='GALFIT')
            if (i == 0) | (i == 2):
        ...
        for i in range(4):
            ...
            if (i == 0) | (i==2):
                if plotbadfits:
                    pl.plot(self.logstellarmass[x_flags[i]],y[x_flags[i]],'kx',markersize=8,label='No Fit')
                if nocolor:
                    sp=pl.scatter(self.logstellarmass[point_flags[i]],y[point_flags[i]],c='k',vmin=0.1,vmax=1,cmap='jet_r',s=60,label='GALFIT')
                else:
                    sp=pl.scatter(self.logstellarmass[point_flags[i]],y[point_flags[i]],c=self.sizeratio[point_flags[i]],vmin=minsize,vmax=maxsize,cmap='jet_r',s=60,label='GALFIT')
            if (i == 1) | (i == 3):
                ...
                if nocolor:
                    pl.scatter(xbin,ybin,c='k',s=300,cmap='jet_r',vmin=minsize,vmax=maxsize,marker='s')
                else:
                    pl.scatter(xbin,ybin,c=sbin,s=300,cmap='jet_r',vmin=minsize,vmax=maxsize,marker='s')
            pl.axis([9.1,11.75,7.e-2,32])
            self.plotelbaz()
            gca().set_yscale('log')
            a=pl.gca()
    ...
    def plotSFRStellarmassNUV24(self,clustername=None):
        ...
        for i in range(4):
            ...
            sp=pl.scatter(self.logstellarmass[point_flags[i]],self.s.SFR_ZDIST[point_flags[i]],c=self.sizeratio[point_flags[i]],vmin=0.1,vmax=1,cmap='jet_r',s=60,label='GALFIT')
            if (i == 0) | (i == 2):
    ...
    def plotSFRvsStellarmass2panel(self,plotsingle=1,showtrunc=0):
        ...
        xbin,ybin,ybinerr=my.binit(self.logstellarmass[flag],self.s.SFR_ZDIST[flag],7)
        xbin,ybin,ybinerr=my.binitbins(9.5,11,6,self.logstellarmass[flag],self.s.SFR_ZDIST[flag])
        plot(xbin,ybin,'bo',markersize=10,label='Exterior')
        errorbar(xbin,ybin,ybinerr,fmt=None,ecolor='b')
        gca().set_yscale('log')
        legend(numpoints=1,loc='upper left',prop={'size':10})
        axis(limits)
        gca().set_yticklabels(([]))
        text(.9,.9,'$Cluster \ vs. \ Exterior $',fontsize=22,transform=gca().transAxes,horizontalalignment='right')
    ...
    def plotSFRvsStellarmass(self,plotsingle=1,plotsize=False,plotagn=False):
        ...
        xbin,ybin,ybinerr=my.binitbins(9.3,11.3,9,x[flag],self.SFR_BEST[flag])
        plot(xbin,ybin,'b^',markersize=12,label='Exterior')
    ...
    def plotsizestellarmass(self,plotsingle=True):
        if plotsingle:
            pl.figure()
            pl.subplots_adjust(bottom=.15,left=.15)
        flag = self.sampleflag & ~self.agnflag
        plot(self.logstellarmass[flag],self.sizeratio[flag],'ko')
        errorbar(self.logstellarmass[flag],self.sizeratio[flag],self.sizeratioERR[flag],fmt=None,ecolor='k')
        xlabel('$ log_{10}(Stellar \ Mass /M_\odot) $',fontsize=20)
        ...
        pl.text(.95,.9,r'$\rho = %4.2f$'%(rho),horizontalalignment='right',transform=ax.transAxes,fontsize=18)
        pl.text(.95,.8,'$p = %5.4f$'%(p),horizontalalignment='right',transform=ax.transAxes,fontsize=18)
        pl.axis([9.3,11.5,0,1.6])
        pl.savefig(homedir+'research/LocalClusters/SamplePlots/sizestellarmass.eps')
        pl.savefig(homedir+'research/LocalClusters/SamplePlots/sizestellarmass.png')
    def newfigure(self):
        ...
        for i in range(2):
            ...
            pl.scatter(self.logstellarmass[flag],self.SFR_BEST[flag],s=50.*self.sizeratio[flag],c='None',vmin=.1,vmax=1,marker=markers[i])
            flag=self.sampleflag  & ~self.agnflag & extraflag[i]
            pl.scatter(self.logstellarmass[flag],self.SFR_BEST[flag],s=50.*self.sizeratio[flag],c=self.NUV24[flag],vmin=1,vmax=4,cmap='jet',marker=markers[i])
    ...
    def plotsmoothnessBT(self):
        ...
        for i in range(len(flags)):
            pl.subplot(1,2,i+1)
            sp=pl.scatter(self.s.B_T_r[flags[i]],self.s.S2g_1[flags[i]],c=self.sizeratio[flags[i]],s=60,vmin=.1,vmax=1,cmap='jet_r')
    ...
    def plotdrBT(self,usesersic=False,useagn=False):
        ...
        sp=pl.scatter(x[flag],self.s.DR_R200[flag],c=self.sizeratio[flag],s=60,vmin=0.1,vmax=1,cmap='jet_r')
        pl.xlabel(xl)
        pl.ylabel('$\Delta R/R_{200} $')
        pl.gca().set_yscale('log')
    ...
    def plotsigmaBTblue(self,usesersic=False):
        ...
        pl.subplots_adjust(left=.15,bottom=.12,wspace=.01,hspace=.03)
        if usesersic:
            x=self.s.SERSIC_N
            limits=[-.1,6.1,.1,40]
        ...
        for i in range(2):
            ...
            if i == 9:
                ...
                ##xbin,sbin,sbinerr=my.binit(self.logstellarmass[point_flags[i]],self.sizeratio[point_flags[i]],5)
                ##errorbar(xbin,ybin,yerr=ybinerr,fmt=None,color='k',markersize=16)
                ##pl.scatter(ybin,10.**xbin,c=sybin,s=300,cmap='jet',vmin=v1,vmax=v2,marker='s')
            ...
            if i == 1:
                ...
                text(-0.2,1.,'$\Sigma_5 \ (gal/Mpc^2)$',transform=a.transAxes,rotation=90,horizontalalignment='center',verticalalignment='center',fontsize=24)
                mstarax.append(a)
                pl.xlabel('$log_{10}(M_*/M_\odot)$',fontsize=20)
                c=colorbar(fraction=.08,pad=.03)
                text(-0.2,1.,'$\Sigma_5 \ (gal/Mpc^2)$',transform=a.transAxes,rotation=90,horizontalalignment='center',verticalalignment='center',fontsize=24)
            if i == 99:
    ...
    def plotLxBT(self,useagn=False,usesersic=False):
        ...
        flag = flag & self.dvflag
        sp=pl.scatter(x[flag],y[flag],c=self.sizeratio[flag],s=60,vmin=0.1,vmax=1,cmap='jet_r')
        pl.xlabel(xl)
    ...
    def plotphiBT(self,useagn=False,usesersic=False):
        ...
        flag = flag & self.dvflag
        sp=pl.scatter(x[flag],self.s.CLUSTER_PHI[flag],c=self.sizeratio[flag],s=60,vmin=0.1,vmax=1,cmap='jet_r')
        pl.xlabel(xl)
        ...
        x=(self.s.SIGMA_5)
        sp=pl.scatter(x[flag],self.s.CLUSTER_PHI[flag],c=(self.sizeratio[flag]),s=60,vmin=.1,vmax=1.,cmap='jet_r')
        pl.xlabel('$\Sigma_5$')
        #pl.gca().set_yscale('log')
        #ylabel('$\Delta R/R_{200} $',fontsize=24)
        pl.text(.95,.9,'$R_e(24)/R_e(r) $',transform=gca().transAxes,horizontalalignment='right',fontsize=16)
    ...
    def plotRe24vsRe(self,plotsingle=1,sbcutobs=20.,prefix=None,usemyflag=0,myflag=None,showerr=0,logy=True,fixPA=False, usedr=False,colorflag=True):
        ...
        if colorflag:
            ...
            plotcolors = ['k','0.5']
        plot(x[mflag ],y[mflag],'ko',color=plotcolors[0],markersize=8)
        plot(x[nfflag ],y[nfflag],'ks',color=plotcolors[1],markersize=8)
        plot(x[ffflag ],y[ffflag],'ks',color=plotcolors[1],markersize=8)
    ...
    def plotRe24vsRev2(self,plotsingle=1,sbcutobs=20.,prefix=None,usemyflag=0,myflag=None,showerr=0):
        ...
        self.addlines()
        text(.1,.9,'$Exterior$',fontsize=18,horizontalalignment='left',transform=ax2.transAxes)
        ...
        hist(fy/fx,bins=mybins,histtype='step',color='blue',hatch='/',label='$Exterior$')#,normed=True)
    ...
    def plotsizehist(self, btcut = None,colorflag=False):
        figure(figsize=(6,6))
        pl.subplots_adjust(left=.15,bottom=.2,hspace=.1)
        axes=[]
        pl.subplot(2,1,1)
        ...
        pl.text(-.2,1,'$N_{gal}$',transform=gca().transAxes,verticalalignment='center',rotation=90,fontsize=24)
        ...
        if btcut == None:
            #pl.ylim(0,20)
            pl.savefig(homedir+'research/LocalClusters/SamplePlots/sizehistblue.eps')
            pl.savefig(homedir+'research/LocalClusters/SamplePlots/sizehistblue.png')
            pl.savefig(homedir+'Dropbox/Research/MyPapers/LCSpaper1/submit/resubmit/fig11a.eps')
        else:
            #pl.ylim(0,15)
            pl.subplot(2,1,1)
            pl.title('$ B/T < %2.1f \ Galaxies $'%(btcut),fontsize=20)
            pl.savefig(homedir+'research/LocalClusters/SamplePlots/sizehistblueBTcut.eps')
            pl.savefig(homedir+'research/LocalClusters/SamplePlots/sizehistblueBTcut.png')
            pl.savefig(homedir+'Dropbox/Research/MyPapers/LCSpaper1/submit/resubmit/fig11b.eps')
    def plotsizeMBThist(self):
        figure(figsize=(12,5))
        pl.subplots_adjust(bottom=.2,wspace=.02)
        axes=[]
        pl.subplot(1,3,1)
        axes.append(pl.gca())
        ...
        hist(self.sizeratio[self.sampleflag & ~self.membflag & ~self.agnflag],bins=mybins,histtype='step',color='blue',hatch='/',label='$Exterior$')
        #pl.title('$ Blue \ Galaxies $',fontsize=20)
        pl.ylabel('$N_{gal}$')
        print 'comparing cluster and exterior SF galaxies'
        ...
        hist(self.logstellarmass[self.sampleflag & ~self.membflag & ~self.agnflag],bins=mybins,histtype='step',color='blue',hatch='/',label='$Exterior$')
        pl.axis([9.3,11.2,0,18])
        #pl.legend(loc='upper right')
        pl.xlabel('$ log_{10} (M_*/M_\odot) $')
        pl.gca().set_yticklabels(([]))

        pl.subplot(1,3,3)
        axes.append(pl.gca())
        mybins=arange(0,.8,.05)
        hist(self.s.B_T_r[self.gim2dflag & self.sampleflag & self.membflag & ~self.agnflag],bins=mybins,histtype='step',color='red',hatch='\\',label='$Core$')
        hist(self.s.B_T_r[self.gim2dflag & self.sampleflag & ~self.membflag & ~self.agnflag],bins=mybins,histtype='step',color='blue',hatch='/',label='$Exterior$')
        pl.axis([-0.05,.8,0,18])
        #pl.legend(loc='upper right')
        pl.xlabel('$ B/T $')
        pl.gca().set_yticklabels(([]))
        ...
        pl.savefig(homedir+'research/LocalClusters/SamplePlots/sizeMBThistblue.eps')
        pl.savefig(homedir+'research/LocalClusters/SamplePlots/sizeMBThistblue.png')
    ...
    def compare(self,var,baseflag=None,plotflag=0,xlab=None,plotname=None):
        ...
        if plotflag:
            ...
            #print var[f2]
            pl.hist(var[f2],bins=len(var[f2]),cumulative=True,histtype='step',normed=True,label='Exterior',range=(xmin,xmax),color='b')
            pl.title('Member vs. Exterior ('+self.prefix+')')
    ...
    def plotagn(self):
         ...
         sp=pl.scatter(x[keepflag],y[keepflag],s=60,c=(self.sizeratio[keepflag]), cmap=cm.jet_r,vmax=1,label='_nolabel_')
         pl.colorbar(sp)
 ...
 #draw AGN diagnostic lines
    ...
    def plotwise(self):
         ...
         pl.subplot(1,2,2)
         sp=pl.scatter(x[keepflag],y[keepflag],s=30,c=(self.sizeratio[keepflag]), cmap=cm.jet_r,vmax=1,label='_nolabel_')
         pl.colorbar(sp)
         pl.axis([1,4.,-.3,.5])
         pl.xlabel(r'$ [5.8] - [8.0]$',fontsize=20)
    ...
    def plotsizedr(self,sbcutobs=20.5,usemass=1,usecolor=0,isoflag=0,blueflag=True):
         ...
         pl.axhline(y=exterior,ls='-',color='k')
         pl.axhline(y=exterior-fstd,ls='--',color='k')
         pl.axhline(y=exterior+fstd,ls='--',color='k')
         #pl.axhline(y=1,ls='-',color='k')
         spearman(x,y)
    ...
    def plotsizeclusterphi2(self,sbcutobs=20.,usemass=0,usecolor=1,useagn=False):
        ...
        sp=pl.scatter(x[flag],y[flag],s=100,c=color[flag],vmin=v1,vmax=v2)
        pl.axhline(y=exterior,ls='-',color='k')
        pl.axhline(y=exterior+exteriorerr,ls='--',color='k')
        pl.axhline(y=exterior-exteriorerr,ls='--',color='k')
    ...
    def plotsizeHIfrac(self,sbcutobs=20.5,isoflag=0,r90flag=0):
         ...
         #color=self.logstellarmass[flag]
         color=self.logstellarmass[flag & self.membflag]
         sp=scatter(x,y,s=90,c=color,vmin=mstarmin,vmax=mstarmax,label='$Core$')
         ...
         #color=self.logstellarmass[flag]
         color=self.logstellarmass[flag & ~self.membflag]
         sp=scatter(x,y,s=90,c=color,vmin=8.5,vmax=10.8,marker='s',label='$Exterior$')
         y=(self.sizeratio[flag])
         ...
         print 'spearman for log(M*) < 10.41'
         rho,p=spearman(x[color < 10.41],y[color<10.41])
         cb = pl.colorbar(sp,fraction=.08,ticks=np.arange(8.5,11,.5))
         ...
         pl.savefig(homedir+'Dropbox/Research/MyPapers/LCSpaper1/submit/resubmit/fig15a.eps')
    def plotsizeHIdef(self,sbcutobs=20.5,isoflag=0,r90flag=0):
         ...
         #pl.ylabel(r'$ R_e(24)/R_e(r)$')#,fontsize=26)
         pl.ylabel('$R_{24}/R_d$')
         pl.xlabel('$HI \ Deficiency$')#,fontsize=26)
         pl.axis([-.6,1.6,0,2.5])
         #ax.tick_params(axis='both', which='major', labelsize=16)
         ...
         pl.savefig(homedir+'Dropbox/Research/MyPapers/LCSpaper1/submit/resubmit/fig15b.eps')
    ...
    def plotsizeclusterphiblue(self):
        ...
        pl.figure()
        pl.subplots_adjust(bottom=.15,left=.15)
        pl.plot(self.s.CLUSTER_PHI[flag],self.sizeratio[flag],'bo')
    ...
    def compareRiso(self):
         ...
         ax2.set_yticklabels(([]))
         axis([0,70,-2,50])
         text(.1,.9,'$Exterior$',transform=ax2.transAxes,horizontalalignment='left',fontsize=18)
         xlabel('$R_{iso}(r) $',fontsize=20)
    ...
    def compare_single(self,var,baseflag=None,plotsingle=True,xlab=None,plotname=None):
        ...
        pl.hist(var[f1],bins=len(var[f1]),cumulative=True,histtype='step',normed=True,label='Core',range=(xmin,xmax),color='k')
            #print var[f2]
        pl.hist(var[f2],bins=len(var[f2]),cumulative=True,histtype='step',normed=True,label='Exterior',range=(xmin,xmax),color='0.5')
        ylim(-.05,1.05)
        ax=gca()
    ...
    def compare_cluster_exterior(self):
        ...
        pl.xticks(np.arange(9,12,.5))
        pl.xlim(8.9,11.15)
        #xlim(mstarmin,mstarmax)
        pl.subplot(2,2,2)
        self.compare_single(self.s.B_T_r,baseflag=(self.sampleflag & ~self.agnflag),plotsingle=False,xlab='$GIM2D \ B/T $',plotname='BT')
        pl.xticks(np.arange(0,1.1,.2))
        pl.xlim(-.05,.85)
        pl.subplot(2,2,3)
        self.compare_single(self.s.ZDIST,baseflag=(self.sampleflag & ~self.agnflag),plotsingle=False,xlab='$ Redshift $',plotname='zdist')
        pl.xticks(np.arange(0.02,.055,.01))
        pl.xlim(.0146,.045)
        pl.subplot(2,2,4)
        ...
        self.compare_single(self.s.Rhlr_2,baseflag=(self.sampleflag & ~self.agnflag),plotsingle=False,xlab='$R_e(r) \ (kpc)$',plotname='Rer')
        ...
        pl.savefig(homedir+'research/LocalClusters/SamplePlots/cluster_exterior.png')
        pl.savefig(homedir+'research/LocalClusters/SamplePlots/cluster_exterior.eps')
        pl.savefig(homedir+'Dropbox/Research/MyPapers/LCSpaper1/submit/resubmit/fig5.eps')
    ...
    def plotcolorcolor(self,colorBT=False,plotnofit=False,colorM=False,colorN=False,plotsingle=True):
        ...
        for i in range(len(flags)):
            ...
            if i == 1:
                ...
                #plt.text(.95,.9,'$Exterior$',transform=gca().transAxes,horizontalalignment='right',fontsize=20)
                plt.title('$Exterior$',fontsize=20)
    ...
    def plotcolorcolor3panel(self,plotnofit=True):
        ...
        for j in range(nrows):
            if j == 0:
                ...
                color=self.s.B_T_r#*self.sizeratio
                v1=0
                v2=.4
                cblabel='$B/T$'
                cc='jet'
    ...
    def plothistssfr(self,minmass=None,maxmass=None,plotsingle=True,sfms=False):
        ...
        pl.hist(log10(self.ssfr[f2]*normfactor),bins=mybins,histtype='step',label='Exterior',color='b',hatch='/',normed=True)
    ...
    def plotsffracvsMstar(self):
        figure(figsize=(6,4))
        subplots_adjust(hspace=.02,wspace=.02,bottom=.2,left=.15)
        i=0
        # bin by stellar mass
        # get fraction of SF
        flag =  ~self.agnflag & self.membflag & (self.logstellarmass > 9.) & (self.logstellarmass < 11.5)
        x = self.logstellarmass[flag]
        lirflag=(self.s.LIR_ZDIST > 2.*5.2e8)
        y = lirflag[flag]
        bins = np.arange(min(x),max(x)+.5,.5)
        nbin = len(bins)
        binnumb=np.array((x-min(x))*nbin/(max(x)-min(x)),'i')
        ybin = np.zeros(nbin,'f')
        ybinerrp = np.zeros(nbin,'f')
        ybinerrm = np.zeros(nbin,'f')        
        for i in range(len(bins)):
            ybin[i],ybinerrp[i],ybinerrm[i] = my.ratioerror(sum(y[binnumb == i]),sum(binnumb == i))
        yerr = np.array(zip(ybinerrp,ybinerrm),'f').T
        plt.errorbar(bins,ybin,yerr=yerr,fmt='ro',label='Core')
        ...
        for i in range(len(bins)):
            ybin[i],ybinerrp[i],ybinerrm[i] = my.ratioerror(sum(y[binnumb == i]),sum(binnumb == i))
        yerr = np.array(zip(ybinerrp,ybinerrm),'f').T
        plt.errorbar(bins,ybin,yerr=yerr,fmt='bo',label='External')
        plt.legend()
        plt.xlabel('$ \log_{10} (M_\star/M_\odot) $',fontsize=20)
        plt.ylabel('$ f_{IR} $',fontsize=20)
...
def plotsizevsMclallwhisker(sbcutobs=20,masscut=False,minmass=10.,drcut=1.,blueflag=False,usetemp=False,useM500=False,usesigma=False,bwflag=True,btcut=None):
    ...
    for cl in clusternamesbylx:
        ...
        if btcut:
            flag = flag & (s.s.B_T_r < 0.3)#& ~s.blueflag
        if blueflag:
            flag = flag & self.blueflag2
        print 'number in ',cl,' = ',sum(flag)
        if masscut:
            flag=flag & (s.logstellarmass < minmass)
        if usetemp:
...
def plotRe24vsReall(sbcutobs=20,plotcolorbar=0,fixPA=False,logyflag=True,usedr=False):
...
#    for cl in clusternamesbydistance:
    ...
    savefig(homedir+'Dropbox/Research/MyPapers/LCSpaper1/submit/resubmit/fig10.eps')
...
def plotsigmahistall():
    ...
    pl.figure(figsize=plotsize_single)
    pl.subplots_adjust(left=.15,bottom=.2,hspace=.02,wspace=.02)
    i=1
    xl='$\Sigma_5 $'
    yl='$\Delta r/R_{200}$'
    allax=[]
    ...
    pl.xlabel('$ \log_{10}(\Sigma_5 \ (gal/Mpc^{-2})) $')
    pl.ylabel('$ Cumulative \ Distribution $')
    pl.axis([-1.2,1.9,-.05,1.05])
    ...
    pl.savefig(homedir+'/research/LocalClusters/SamplePlots/sigmahist.eps')
    pl.savefig(homedir+'/research/LocalClusters/SamplePlots/sigmahist.png')
...
def plotsigmaLx(bwflag=True):
    ...
    savefig(homedir+'Dropbox/research/MyPapers/LCSpaper1/submit/resubmit/fig1.eps')
def computePCA():
...
def paperTable1(sbcutobs=20,masscut=0):
    ...
    #outfile.write('\\tablehead{\colhead{Cluster} &\colhead{Biweight Central Velocity} & \colhead{Lit.} & \colhead{Biweight Scale} & \colhead{Lit} & \colhead{N$_{spiral}$} & \colhead{N$_{spiral}$} } \n')#  % \\\\ & \colhead{(km/s)}  & \colhead{(km/s)} & \colhead{(km/s)}  & \colhead{(km/s)} & \colhead{Member} & \colhead{Exterior}} \n')
    outfile.write('\\tablehead{\colhead{Cluster} &\colhead{Biweight Central Velocity}  & \colhead{Biweight Scale} & \colhead{N$_{gal}$} & \colhead{N$_{gal}$}& \colhead{N$_{gal}$} \\\\ & \colhead{(km/s)}  & \colhead{(km/s)}  & Member & Near-Exterior & Exterior } \n')
    outfile.write('\startdata \n')
...
def paperTable1Paper1(sbcutobs=20,masscut=0):
    ...
    #outfile=open(homedir+'/Dropbox/Research/MyPapers/LCSpaper1/Table1.tex','w')
    outfile=open(homedir+'/Dropbox/Research/MyPapers/LCSpaper1/submit/resubmit/Table1.tex','w')
    ...
    #outfile.write('\\tablehead{\colhead{Cluster} &\colhead{Biweight Central Velocity} & \colhead{Lit.} & \colhead{Biweight Scale} & \colhead{Lit} & \colhead{N$_{spiral}$} & \colhead{N$_{spiral}$} } \n')#  % \\\\ & \colhead{(km/s)}  & \colhead{(km/s)} & \colhead{(km/s)}  & \colhead{(km/s)} & \colhead{Member} & \colhead{Exterior}} \n')
    outfile.write('\\tablehead{\colhead{Cluster} &\colhead{Biweight Central Velocity}  & \colhead{Biweight Scale} & \colhead{N$_{gal}$} & \colhead{N$_{gal}$} \\\\ & \colhead{(km/s)}  & \colhead{(km/s)}  & Core & Exterior } \n')
    outfile.write('\startdata \n')
...
def plotsizevscluster():
    ...
    btcut = .3
    flag = s.sampleflag & (s.s.B_T_r < btcut) & s.membflag
    ...
    exteriorflag = s.sampleflag & (s.s.B_T_r < btcut) & ~s.membflag & ~s.dvflag
    nearexteriorflag = s.sampleflag & (s.s.B_T_r < btcut) & ~s.membflag & s.dvflag
    ...
    for i in range(len(envs)):
        ...
        y2.append(np.median(s.s.B_T_r[envs[i]]))
        #ypoint.append(ws.weighted_mean(s.sizeratio[envs[i]],weights=1./s.sizeratioERR[envs[i]]))
        y2err.append(np.std(s.s.B_T_r[envs[i]])/np.sqrt(1.*np.sum(envs[i])))
    ...
    plt.xticks(np.arange(len(envs)),['$Field$', '$Near-Field$', '$Group$', '$Cluster$', '$Coma$'],fontsize=18)
    ...
    pl.savefig(homedir+'research/LocalClusters/SamplePlots/size_env.eps')
    pl.savefig(homedir+'research/LocalClusters/SamplePlots/size_env.png')
    pl.savefig(homedir+'Dropbox/Research/MyPapers/LCSpaper1/submit/resubmit/fig14.eps')
