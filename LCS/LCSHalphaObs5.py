...
ram=array([25,42,47,55,24,31,05,27,41,6,6,54,18],'f')
ras=array([50.2,50,0,0,0,0,57.,1.9,13,36,11.8,14.1,0],'f')
#ras=array([12.2,9.4,51.8,30.5,02.6,31.2,57.,1.9,49.1,32.,41.8,14.1,15.4],'f')

decd=array([11,10,13,25,9,10,9,25,3,1,14,25,35],'f')
decm=array([16,25,56,50,14,47,15,40,49,45,26,0,24],'f')
decs=array([49,0,0,0,0,19,30,10,11,00,49,11.8,30],'f')
groupra=(rah+ram/60.+ras/3600)*15.
groupdec=decd+decm/60.+decs/3600.

rah2=array([9,11,11,11,12,13,14,14,14,15,10,13,16],'f')
ram2=array([33,43,47,58,28,28,1,31,40,0,7,50,16],'f')
ras2=array([25,48,49,10.5,02.6,43,57.,1.9,49.1,43.2,0,14.1,0],'f')

decd2=array([9,9,12,24,9,12,9,25,2,1,13,24,34],'f')
decm2=array([54,0,50,54,9,41,15,40,49,54,26,55,25],'f')
decs2=array([0,0,0,23,30,19,58,10,11,0,0,30,0],'f')
groupra2=(rah2+ram2/60.+ras2/3600)*15.
groupdec2=decd2+decm2/60.+decs2/3600.


rah3=array([9,11,11,11,12,13,14,14,14,15,10,13,16],'f')
ram3=array([38,46,47,58,25,29,10,28,37,11,7,46,16],'f')
ras3=array([25,45,49,10.5,12.6,20,57.,0.9,9,6.0,0,14.1,0],'f')

decd3=array([9,10,12,24,8,11,8,27,3,1,13,24,34],'f')
decm3=array([5,45,50,54,9,47,45,0,49,42,26,55,25],'f')
decs3=array([0,0,30,23,30,19,30,0,11,0,0,30,0],'f')

groupra3=(rah3+ram3/60.+ras3/3600)*15.
groupdec3=decd3+decm3/60.+decs3/3600.

rah4=array([9,11,11,11,12,13,14,14,14,15,10,13,16],'f')
ram4=array([38,46,47,58,25,26,10,28,37,11,7,50,22],'f')
ras4=array([25,45,49,10.5,12.6,0,57.,0.9,9,6.0,0,14.1,0],'f')

decd4=array([9,10,12,24,8,12,8,27,3,1,13,24,38],'f')
decm4=array([5,45,50,54,9,10,45,0,49,42,26,55,0],'f')
decs4=array([0,30,0,23,30,0,30,0,11,0,0,30,0],'f')
...
# trying to adjust pointings for MKW11 assuming that we will have only 2 pointings
#13 29 31.2   +11 47 19
rah=array([9,11,11,11,12,13,14,14,14,15,10,13,16],'f')
ram=array([25,42,47,55,24,31,05,27,41,6,6,54,18],'f')
ras=array([50.2,50,0,0,0,0,57.,1.9,13,36,11.8,14.1,0],'f')
...
#for i in range(1):
    ...
    #write regions files
    s=groupnames[i]+'.halpha.reg'
    s2='/Users/rfinn/research/LocalClusters/RegionsFiles/'+s
    outfile3=open(s2,'w')
    ...
    for j in range(len(ra)):
        if flag[j]:
            s='circle(%12.8f, %12.8f, 30") \n'%(ra[j],dec[j])
            outfile3.write(s)
        if flagmemb[j]:
            s='circle(%12.8f, %12.8f, 40") # color=green \n'%(ra[j],dec[j])
            outfile3.write(s)
    outfile3.close()
