...
'''
 ...
 MODIFICATION HISTORY:
   ...
   R. Finn, 2013-07-03, converted idl to python
...
def lf_double_powerlaw(loglum, phistar, loglstar,alpha, beta):#, powerlaw=powerlaw)

    if (len(loglum) == 0):
       print 'lf_double_powerlaw'
       return -1

    #if (len(powerlaw) == 0):
    #   if (len(phistar) eq 0) or (len(loglstar) eq 0) or (len(alpha) eq 0) or (len(beta) eq 0):
    #      print 'PHISTAR, LOGLSTAR, ALPHA, and BETA inputs required'
    #      return -1
    use_phistar = phistar
    use_loglstar = loglstar
    use_alpha = alpha
    use_beta = beta
    # not sure what this next part is doing so skipping for now
    #elif:
    ...
    intlf = use_phistar*(10.**(loglum-use_loglstar))**(-1*use_alpha)*(1.0+10.**(loglum-use_loglstar))**(-1*use_beta)
    model = log(10)*10.**loglum*(use_alpha/10.**loglum+use_beta/(10.**loglum+10.**use_loglstar))*intlf
    
    return model

def intlf_double_powerlaw(loglum, phistar, loglstar,alpha, beta):#, powerlaw=powerlaw)

    #if (len(loglum) == 0):
    #   print 'intlf_double_powerlaw'
    #   return -1
