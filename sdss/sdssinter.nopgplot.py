    ...
    for i in range(len(g.dr)):
        ...
        if (g.memb[i] > 0):
            if (g.ew[i] > -500.):#make sure it's in dr2
                if (g.Mabs[i] < mabscut):
                    final.append(1.)
                    dr.append(g.dr[i]/g.clusterrvir[i])
...
def plotsfrmabs():
    ...
    yl=N.arange(.1,40.,.1)
    xl=-20.4*N.ones(len(yl))
    pylab.plot(xl,yl,'k--')
    pylab.axis([-22.5,-18.5,0.5,9.])
    pylab.setp(ax,xticklabels=['','-22','','-21','','-20','','-19',''])
...
def plotewmabshiz():
    ...
    x=N.compress((g.Mabs > -999.) & (g.Mabs < 0.) &  (g.agn < 1) & (g.dr < 0.5*g.clusterrvir) & (g.dv < 3.*g.clustersigma) & (g.ew > 4.),g.Mabs)

    y=N.compress((g.Mabs > -999.) & (g.Mabs < 0.) &  (g.agn < 1) & (g.dr < 0.5*g.clusterrvir) & (g.dv < 3.*g.clustersigma) & (g.ew > 4.),g.ew)
    ...
    ppgplot.pgbox("",0.0,0,"",0.0,1)
    #ppgplot.pgenv(xmin,xmax,ymin,ymax,0,20)
    ppgplot.pgenv(ymin,ymax,xmin,xmax,0,10)
    ppgplot.pglab(ylabel,xlabel,"")
    ppgplot.pgsci(1)
...
class Cluster:
    ...
    def recordchisqfield(self):
        ...
        self.voffset=N.zeros(len(self.z),'f')
	chisqdat=open("/Users/rfinn/SDSS/clusterDR5/chisq.dat",'r')
	id=[]
	chisq=[]
...
#        self.sumsfr2 = N.zeros(len(self.sigma),'f')
    ...
    def initializecompleteness(self):
...
def plotvelhist():#plot histogram of velocities with gaussian
    ...
    for i in range(len(c.id)):
        ...
        chisq[i]=velhistsub2(bins,dv2,sigma,i,1)
	if (chisq[i] > chisqmax):
	    dvel2 = N.compress((abs(g.clusterid - c.id[i]) < 1) & (g.dr < 2.*c.r200[i]) & (abs(g.dv) < 2.*c.sigma[i]), g.dv)
	    velave=N.average(dvel)#try calculating new center for cluster
	    dvel=dvel+-1.*velave
	    dv2=dvel/sigma
	    chisq[i]=velhistsub2(bins,dv2,sigma,i,2)
	    if (chisq[i] < chisqmax):
		print "Recentered cluster ",c.id[i], chisq[i]
		voffset[i]=-1.*velave
	string="%6.1f %6.1f %8.2f %8.4f \n"%(c.id[i],c.sigma[i],chisq[i],voffset[i])
...
def writefiles():
    ...
    for i in range(len(c.z)):
        ...
        output.write('%5.0f %5.4f %5.3f %5.3f %6.1f %6.1f %6.3f %2.0f %3.2e %3.2e %3.2e %3.2e %5.1f %5.1f %5.1f %5.1f %4.2f %4.2f %4.2f %2.0f %4.3e %4.3e %4.3e %5.3f %5.3f %5.1f %5.1f %5.1f %5.1f %3.2e %3.2e %3.2e %3.2e %5.2f %3.2e %5.3f %12.8f %12.8f %6.1f  %6.1f  %6.1f  %8.3f %8.3f %8.3f %8.3f %8.3f %8.3f %8.3f %8.3f %8.3f \n' %(c.id[i],c.z[i],c.rvir[i],c.r200[i],c.sigma[i],c.totrlum[i],c.Lx[i],c.xrayflag[i],c.stellmass05[i],c.stellmass1[i],c.stellmass2[i],c.stellmass3[i],c.sfr05[i],c.sfr1[i],c.sfr2[i],c.sfr3[i],c.sffrac05[i],c.sffrac1[i],c.sffrac2[i],c.sub[i],c.totlum05[i],c.totlum1[i],c.totlum2[i],c.r200bin[i],c.sigmabin[i],c.sfr05bin[i],c.sfr1bin[i],c.sfr2bin[i],c.sfr3bin[i],c.stellmass05bin[i],c.stellmass1bin[i],c.stellmass2bin[i],c.stellmass3bin[i],c.chisq[i],c.sfr05hiz[i],c.kauffmann[i],c.ra[i],c.dec[i],c.ngal05[i],c.ngal1[i],c.ngal2[i],c.compl05[i],c.compl1[i],c.compl2[i],c.voffset[i],c.nsf05hiz[i],c.n05hiz[i],c.sffrac05a[i],c.sffrac1a[i],c.sffrac2a[i]))
...
if (mode == 1):
	...
	print "reading in completeness"
        #c.readsdsscompleteness("sdsscompleteness0.dat")
	c.readsdsscompleteness()
...
def doagn(dv,dr):
    ...
    print "Fraction (average) of ha luminosity due to agn = %5.2f within %3.1f Rv, %3.1f sigma" % ((1.-N.average(totalsfr)/N.average(totalsfrall)),dr,dv)
    ...
    print "Fraction by number (average) of agn = %5.2f within %3.1f Rv, %3.1f sigma" % (agnfrac,dr,dv)
    print "********************************************"
print " "
print "*** AGN STATS  ***"
print " "
