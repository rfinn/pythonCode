...
def sfrmabsmulti():
    ...
    for i in range(len(allm)):#all galaxies
	if ((allm[i]+dm) < (mabscut+dm)):
	    ntot3=ntot3+1
	if ((allm[i]) < mabscut):
	    ntot4=ntot4+1

    for i in range(len(sxt)):#sf galaxies only
	sxt[i]=float(sm[i])
	syt[i]=float(ssfr[i])
	if (syt[i] >  (sxt[i]*m +b)):
	    smsub.append(sxt[i])
    ...
    print "BIANCA COMPARISON: SF frac for sfr scale only", float(len(ssfrsub4))/float(ntot4) 
    print "BIANCA COMPARISON: SF frac for sfr scale & brighten", float(len(ssfrsub3))/float(ntot3) 
    esfr=N.log10(esfr)
    #ppgplot.pgsci(2)
    ppgplot.pgsci(1)
...
def writefiles():
    ...
    output.write(header)
    header="#cid cz crvir cr200 csigma(km/s) dr(Mpc) dv(km/s) ew(A) lHa(1e40 erg/s) stellmass agnflag sfflag memb Mabs RA(deg) Dec(deg) ar mpaflag"
    output.write("%s \n" % (header))
