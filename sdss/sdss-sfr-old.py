...
masshiz=12*(sigmahiz/1000.)**3.*1./N.sqrt(.7+.3*(1+zhiz))
mratio=masshiz/kodamamasshiz
sfrmhiz=(sumsfrhiz/kodamamasshiz)*(.64)#correct for AGN contamination
#for i in range(len(zhiz)):
#    print i, zhiz[i],masshiz[i],sfrmhiz[i],mratio[i]

#completeness cuts
version=1
if version == 1:
    mabsorig=-21.36#store original
    mabscut=-21.36 #corresponds roughly to r=17.5 at z=0.17
    ...
    zmax=0.9 #min redshift of sample
...
def sumit(sig,r,agn):
    ...
    if agn < 1:
        for i in range(len(y)):
            y[i]=N.sum(N.compress((abs(g.clusterid - c.id[i])< 1) & (g.ew > ewmin)  & (g.dv < sig*c.sigma[i]) & (g.dr < r*c.rvir[i]) & (g.Mabs < mabscut) & (g.agn < 1) & (g.lHa > lHamin), g.sfr))#keep and then sum g.sfr according to selection criteria on cluster, sfr, stellarmass, dv, and dr
    if agn > 0:#include agn in summed sfr
        for i in range(len(y)):
            y[i]=N.sum(N.compress((abs(g.clusterid - c.id[i])< 1) & (g.ewr > ewmin)  & (g.dv < sig*c.sigma[i]) & (g.dr < r*c.rvir[i]) & (g.Mabs < mabscut) & (g.lHa > lHamin), g.sfr))#keep and then sum g.sfr according to selection criteria on cluster, sfr, stellarmass, dv, and dr
...
def sumstellmass(sig,r):
    y=N.zeros(len(c.z),'f')
    for i in range(len(y)):
        y[i]=N.sum(N.compress((abs(g.clusterid - c.id[i])< 1) & (g.stellarmass > 0) & (g.dv < sig*c.sigma[i]) & (g.dr < r*c.rvir[i]) & (g.Mabs < mabscut), g.stellarmass))#keep and then sum g.sfr according to selection criteria on cluster, sfr, stellarmass, dv, and dr
    return y
...
def richness(sig,r,agn,mabs):
    ...
    if agn < 1:
        for i in range(len(y)):
            y[i]=len(N.compress((abs(g.clusterid - c.id[i])< 1) & (g.dv < sig*c.sigma[i]) & (g.dr < r*c.rvir[i]) & (g.Mabs < mabs) & (g.agn < 1), g.sfr))#keep and then sum g.sfr according to selection criteria on cluster, sfr, stellarmass, dv, and dr
    if agn > 0:#include agn in summed sfr
        for i in range(len(y)):
            y[i]=len(N.compress((abs(g.clusterid - c.id[i])< 1) & (g.dv < sig*c.sigma[i]) & (g.dr < r*c.rvir[i]) & (g.Mabs < mabs), g.sfr))#keep and then sum g.sfr according to selection criteria on cluster, sfr, stellarmass, dv, and dr
...
def plotmclz():
    ...
    ppgplot.pgenv(0.,0.2,0.,1.6,0,20)
...
def plotsfrpmclsigma():
    ...
    y=(c.sfrmass)
    ppgplot.pgbox("",0.0,0,"",0.0,0)
    ppgplot.pgenv(0,1700,0,1000,0)#match hiz axes
...
def plotsfrpmclz():
    ...
    ymin=min(y)
    ppgplot.pgbox("",0.0,0,"",0.0,0)
    ppgplot.pgenv(0,0.17,-1,ymax,0,0)
...
def plotsfrmabs():
    ...
    ymin=min(y)
    ppgplot.pgbox("",0.0,0,"",0.0,1)
    ppgplot.pgenv(xmin,xmax,-.5,10,0,0)
    ppgplot.pglab("M\dR\u","SFR (M\d \(2281)\u yr\u-1\d)","")
    ppgplot.pgpt(x,y,1)

def plotewmabs():
    x=N.compress((g.Mabs > -999.) & (g.Mabs < 0.)& (g.sfr > -998.) & (g.ew > -998.) & (g.agn < 1),g.Mabs)
...
class Cluster:
    ...
    def limitrichness(self):
        ...
        for i in range(len(self.id)):
            self.ngal[i] = len(N.compress((abs(g.clusterid - c.id[i])< 1) & (g.Mabs < mabscut), g.sfr))
    ...
    def calcrichness(self):
        ...
        for i in range(len(self.richness)):
            self.richness[i]=len(N.compress((abs(g.clusterid - self.id[i])< 1) & (g.dv < nsig*self.sigma[i]) & (g.dr < nr*self.rvir[i]) & (g.Mabs < mabscut), g.sfr))#keep and then sum g.sfr according to selection criteria on cluster, sfr, stellarmass, dv, and dr
            #self.richness[i]=len(N.compress((abs(g.clusterid - self.id[i])< 1) & (abs(g.dv/self.sigma[i])  < (3.-g.dr/self.rvir[i]))  & (g.Mabs < mabscut), g.sfr))#keep and then sum g.sfr according to selection criteria on cluster, sfr, stellarmass, dv, and dr
            self.totlum[i]=N.sum(N.compress((abs(g.clusterid - self.id[i])< 1) & (g.dv < nsig*self.sigma[i]) & (g.dr < nr*self.rvir[i]) & (g.Mabs < mabscut), g.rlum))#keep and then sum g.sfr according to selection criteria on cluster, sfr, stellarmass, dv, and dr
...
#        self.sumsfr2 = N.zeros(len(self.sigma),'f')
    ...
    def calcmass(self):
        self.mass=12*(self.sigma/1000.)**3.*1./N.sqrt(.7+.3*(1+self.z))
    def calcr200(self):
        self.r200=N.zeros(len(self.z),'f')
        self.r200=1.73*self.sigma/1000.*1./N.sqrt(.7+.3*(1+self.z))
...
#            if i < 20:
#                print fields[0], fields[1], fields[2],self.agn1[i],self.agn2[i],self.agn3[i]
            i += 1
        #self.lR=3.826E33*10**((4.76-self.Mabs)/2.5)
        #for i in range(len(self.lHa)):
        #    self.lR[i]=float(2.826*pow(10.,33)*pow(10.,((4.76-self.Mabs[i])/2.5))/pow(10.,40.))
            #print self.lHa[i],self.lR[i]
        #self.ew=self.lHa/self.lR*1400.
        #print self.ew
...
c=Cluster()
g=Galaxy()
clusters = glob.glob("cluster_DR2*")
l=len(clusters)
#print clusters
i=len(clusters)
#galaxy=zeros((i,600),'f')
k=0
...
#    print i,clusters[i], "number of lines = ",nlines
#    if nlines < (35 + 14):
#        file.close()
#        print "too few members in cluster ",clusters[i]
#        continue
#    else:
#        file.close()
#    for lines in open(clusters[i]).readlines():
#        print "check lines ",len(lines)
#        if len(lines) < (55 + 14):    #14 header lines
#            print "too few members"
#            break
#        continue
#    print "continuing with cluster ",clusters[i]
    for line in open(clusters[i]):
        ...
        if j == 0:
            fields=line.split()
            sum=0
            if float(fields[5]) < sigmamin:#skip clusters with masses below sigmamin
                break
#            if float(fields[3]) < zmin:#skip clusters with z below zmin
#                print i,"z too low"
#                break
            c.id.append(int(fields[0]))
            c.ra.append(float(fields[1]))
            c.dec.append(float(fields[2]))
            c.z.append(float(fields[3]))
            c.rvir.append(float(fields[4]))#Rvirial in Mpc
            c.sigma.append(float(fields[5]))#velocity dispersion in km/s
            j=1
            id=int(fields[0])
        ...
        if line.find('*') > -1: #skip any galaxies w/ ****** in one or more fields
            continue
        fields=line.split()
        g.clusterid.append(id)#keep track of cluster id
        g.clusterrvir.append(virial)#keep track of cluster virial radius
        g.clustersigma.append(sigma)#keep track of cluster sigma
        g.clusterz.append(z)#keep track of cluster sigma
        #print z
        g.ra.append(float(fields[0]))
        g.dec.append(float(fields[1]))
        g.z.append(float(fields[2]))
        g.Mabs.append(float(fields[3]))
        g.dr.append(float(fields[4]))#projected radial distance in Mpc
        g.dv.append(float(fields[5]))#km/s
        g.dLMpc.append(float(fields[6]))#luminosity distance in Mpc
        g.fHa.append(float(fields[7]))#flux of Halpha in 1E-17 ers/s/cm@
        g.lHa.append(float(fields[8]))#luminosity of Halpha in 1E40 erg/s
        g.apcor.append(float(fields[9]))#aperture correction from flux_R(from image)/flux_R(in fiber)
        g.stellarmass.append(float(fields[10]))#log10(Msun)
        g.az.append(float(fields[11]))#magnitudes of extinction
        g.O3Hb.append(float(fields[12]))#lg(OIII/Hbeta) - AGN diagnostic
        g.N2Ha.append(float(fields[13]))#lg(NII/Halpha) - AGN diagnostic
        if float(fields[8]) > (-100.): #get rid of large negative values
            sum += float(fields[8])
    c.sumsfr.append(float(sum))

            
#convert cluster properties to arrays
g.convarray()
g.calcew()
g.calcagn()
g.calcrlum()
...
g.calcmyapcor()
g.calcforplots()
g.sfr=N.clip(g.sfr,0,100000)
#print g.agn
c.convarray()
c.limitz()#0.05 < z < 0.09
c.limitrichness()#keep only clusters with > 35 members
c.calcmass()
c.calcr200()
c.calcsumsfr2()
c.calcrichness()
...
dostats()
#for i in range(len(c.sumsfr)):
#    print c.id[i], c.sumsfrbalogh[i], c.sumsfr[i], c.sumsfrhiz[i]
#x=array(c.sigma,'f')
#y=array(c.sumsfr,'f')
...
if plotall == 0 or plotall == 2:
    #plot cluster richess vs z 
    ppgplot.pgbeg("richnessvsz.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    plotrichnessz()
    #plot R200 versus Rvir
    ppgplot.pgbeg("r200rv.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    plotr200rv()
    #plot R200 versus Rvir
    ppgplot.pgbeg("cumulativesfrlowz.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    plotcumulativesfrlowz()
    ppgplot.pgbeg("cumulativesfrewlowz.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    plotcumulativesfrewlowz()
#plot cluster mass vs z 
    ppgplot.pgbeg("mclvsz.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    plotmclz()
    ...
    #plot cluster mass vs z 
    ppgplot.pgbeg("magcutz.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    plotmagcutz()
    ...
    #plot cluster mass vs z 
    ppgplot.pgbeg("richnesssigma.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    plotrichnesssigma()
    ...
    #plot cluster mass vs z 
    ppgplot.pgbeg("richnessr.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    plotrichnessr()

    ppgplot.pgbeg("azvsmabs.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    plotazmabs()

    ppgplot.pgbeg("stellarmassvsmabs.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    plotstellarmassmabs()
    ...
    ppgplot.pgbeg("sfrvsmcl.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    plotsfrmcl()
    #plot sumsfr versus cluster mass, 3 sigma cuts
    #ppgplot.pgbeg("sfr3sigmas.ps/vcps",1,1)
    #ppgplot.pgpap(8.,1.25)
    #ppgplot.pgsch(1.7) #font size
    #ppgplot.pgslw(4)  #line width
    #plot3sigmas()
    #plot sumsfr versus cluster mass, 3 radial cuts
    #ppgplot.pgbeg("sfr3r.ps/vcps",1,1)
    #ppgplot.pgpap(8.,1.25)
    #ppgplot.pgsch(1.7) #font size
    #ppgplot.pgslw(4)  #line width
    #plot3r()
    #plot dv/sigma versus projected r/rvir
    ppgplot.pgbeg("dvr.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    plotdvvsr()
    #plot sumsfr versus stellar mass
    ppgplot.pgbeg("stellarmassz.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    plotstellarmassz()
    #plot sumsfr versus stellar mass
    ppgplot.pgbeg("sfrvsstellarmass.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    plotsfrstellarmass()
    #plot sumsfr/stellarmass versus cluster mass
    ppgplot.pgbeg("sfrperstellarmassvsmcl.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    plotsfrpstellarmassmcl()
    #plot sum stellarmass versus cluster mass
    ppgplot.pgbeg("stellarmassvsmcl.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    plotstellarmassmcl()
    #plot sumsfr/stellarmass versus z
    ppgplot.pgbeg("sfrperstellarmassvsz.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    plotsfrpstellarmassz()
    #plot sumsfr/Mcl versus sigma
    ppgplot.pgbeg("sfrmasssigma.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    plotsfrpmclsigma()
    #plot sumsfr/Mcl versus cluster mass
    ppgplot.pgbeg("sfrpermclvsmcl.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    plotsfrpmclmcl()
    #plot sfr per cluster mass vs z for 4 radial cuts
    ppgplot.pgbeg("sfrzallr.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    plotsfrzallr()
    #plot sfr per cluster mass vs z for 4 radial cuts
    ppgplot.pgbeg("sfrzalls.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    plotsfrzalls()
    ppgplot.pgbeg("sffracsfrmcl.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    plotsffracsfrmcl()
    #plot sfr per cluster mass vs z
    ppgplot.pgbeg("sfrpermclvsz.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    plotsfrpmclz()
    #plot sfr per stellar mass versus r(projected)/rvir
    ppgplot.pgbeg("sfrstellarmassvfracrvir.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    plotsfrpstellarmassfracrvir()
    #plot sfr per mcl versus z, including hiz clusters
    ppgplot.pgbeg("hizz.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    plothizz()

    #plot sfr per mcl versus sigma, including hiz clusters
    ppgplot.pgbeg("hizsigma.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    plothizsigma()
    ...
    #plot sfr per mcl versus sigma, including hiz clusters
    ppgplot.pgbeg("hizmcl.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    plothizmcl()

    ppgplot.pgbeg("sffracz.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    plotsffracz()

    ppgplot.pgbeg("avesfrz.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    plotavesfrz()
...
#    plothistmabs()
    ...
    ##############################
    #plot sumsfr/mcl versus mcl for z~.08 clusters only
    ppgplot.pgpage
    ppgplot.pgbeg("sfrmclmcl08.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    ...
    ppgplot.pgpage
    ppgplot.pgbeg("sffracmcl08.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    ...
    ppgplot.pgpage
    ppgplot.pgbeg("sffracsigma.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    ...
    ppgplot.pgpage
    ppgplot.pgbeg("sffracmcl.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    ...
    #plot sumsfr/mcl versus sigma w/hi-z clusters
    ppgplot.pgpage
    ppgplot.pgbeg("sfrmabs.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    plotsfrmabs()

    ppgplot.pgbeg("ewmabs.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    plotewmabs()
    ...
    ppgplot.pgbeg("ewvsfracrvir.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    ...
    ppgplot.pgbeg("ewvsfrperstellarmass.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    ppgplot.pgbox("",0.0,0,"",0.0,1)
    ...
    ppgplot.pgbeg("sumsfrvsumsfr2.ps/vcps",1,1)
    ppgplot.pgpap(8.,1.25)
    ppgplot.pgsch(1.7) #font size
    ppgplot.pgslw(4)  #line width
    ppgplot.pgbox("",0.0,0,"",0.0,1)
